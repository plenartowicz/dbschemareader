...
namespace DatabaseSchemaReader.SqlGen.SqlServer
{
	class SqlServerMigrationGenerator : MigrationGenerator
	{
		...
		public override string AddIndex(DatabaseTable databaseTable, DatabaseIndex index)
		{
			...
			if (sqlServerIndex?.IncludedColumns.Any() == true)
			{
				var columnList = GetColumnList(sqlServerIndex.IncludedColumns.OrderBy(x => x.Ordinal).ThenBy(x => x.Name).Select(x => x.Name));
				sb.AppendLine();
				sb.AppendFormat("INCLUDE({0})", columnList);

			}
			if (!string.IsNullOrEmpty(index.Filter))
			{
				sb.AppendLine();
				sb.AppendFormat("WHERE {0}", index.Filter);
			}

			if (sqlServerIndex?.FillFactor > 0 && sqlServerIndex.FillFactor < 100)
			{
				sb.AppendLine();
				sb.AppendFormat("WITH (FILLFACTOR = {0})", sqlServerIndex.FillFactor);
			}
