...
namespace DatabaseSchemaReader.SqlGen
{
	class MigrationGenerator : IMigrationGenerator
	{
		...
		protected string GetIndexedColumnList(IEnumerable<DatabaseIndexedColumn> columns)
		{
			var columnNames = columns.Select(column => FormatName(column)).ToArray();
			return string.Join(", ", columnNames);
		}

		protected virtual string FormatName(DatabaseIndexedColumn column)
		{
			return Escape(column.Name) + (column.ColumnOrder.HasValue ? " " + column.ColumnOrder.Value : "");
		}
	}
}
